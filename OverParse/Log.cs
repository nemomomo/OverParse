    ...
    public class Log
    {
        ...
        public Log(string attemptDirectory)
        {
            ...
            if (Properties.Settings.Default.BanWarning)
            {
                ...
                MessageBoxResult panicResult = MessageBox.Show("OverParseはPSO2の利用規約を破るサードパーティ製のツールです。セガはparsing toolsを使用されていることが確認された場合、アカウントを停止すると公式サイトで発表しています。\n\n最優先事項がアカウントの安全性である場合は、OverParseを使用しないでください。あなたは、全て自己責任の上でこのツールを使用します。\n\nセットアップを続行しますか？", "OverParse Setup", MessageBoxButton.YesNo, MessageBoxImage.Question);
            ...
            while (!File.Exists($"{attemptDirectory}\\pso2.exe"))
            {
                ...
                if (nagMe)
                {
                    ...
                    MessageBox.Show("有効なpso2_binディレクトリではありません\nデフォルトの設定を使用してゲームをインストールした場合\n\nDrive:\\Program File (x86)\\SEGA\\PHANTASYSTARONLINE2\\pso2_bin\\", "OverParse Setup", MessageBoxButton.OK, MessageBoxImage.Warning);
                }
                else
                {
                    ...
                    MessageBox.Show("pso2_binディレクトリを選択してください。 OverParseがダメージログを読むために使用します。\nこのフォルダは、PSO2インストールフォルダの中にあります。", "OverParse Setup", MessageBoxButton.OK, MessageBoxImage.Information);
                    nagMe = true;
                }
                ...
                dialog.Description = "pso2_binフォルダを選択してください。 PSO2インストールフォルダの中にあります。";
